...
#>
	...
	$global:ReferentialList = @{SourceRefObjs = [System.Collections.ArrayList]@(); DestRefObjs = [System.Collections.ArrayList] @();
		...
		SourceRefSets = [System.Collections.ArrayList]@(); DestRefSets = [System.Collections.ArrayList]@();
		SourceRefWfs = [System.Collections.ArrayList]@(); DestRefWfs = [System.Collections.ArrayList]@();
	}
	$global:SourceObjects = @{WorkflowDefinition = [System.Collections.ArrayList]@(); Set = [System.Collections.ArrayList]@(); }
	$global:TargetObjects = @{WorkflowDefinition = [System.Collections.ArrayList]@(); Set = [System.Collections.ArrayList]@(); }
...
#>
	...
	foreach ($objt in $setsSrc) {
		$global:ReferentialList.SourceRefSets.Add($objt) | Out-Null
	}
	foreach ($obj in $setsSrc) {
		$global:SourceObjects.Set.Add($obj) | Out-Null
	}
	...
	foreach ($objt in $workflowSrc) {
		$global:ReferentialList.SourceRefWfs.Add($objt) | Out-Null
	}
	foreach ($obj in $workflowSrc) {
		$global:SourceObjects.WorkflowDefinition.Add($obj) | Out-Null
	}
	...
	foreach ($objt in $setsDest) {
		$global:ReferentialList.DestRefSets.Add($objt) | Out-Null
	}
	foreach ($obj in $setsDest) {
		$global:TargetObjects.Set.Add($obj) | Out-Null
	}
	$workflowDest = Get-ObjectsFromConfig -ObjectType WorkflowDefinition
	foreach ($objt in $workflowDest) {
		$global:ReferentialList.DestRefWfs.Add($objt) | Out-Null
	}
	foreach ($obj in $workflowDest) {
		$global:TargetObjects.WorkflowDefinition.Add($obj) | Out-Null
	}
...
Function Compare-MimObjects {
	...
	foreach ($obj in $ObjsSource) {
		...
		if (!$obj2) {
			...
			$obj = Resolve-Reference -Object $obj -ObjectType Set -Property PrincipalSet
			$obj = Resolve-Reference -Object $obj -ObjectType Set -Property ResourceCurrentSet
			$obj = Resolve-Reference -Object $obj -ObjectType Set -Property ResourceFinalSet
			$obj = Resolve-Reference -Object $obj -ObjectType WorkflowDefinition -Property ActionWorkflowDefinition -IsArray
			$obj = Resolve-Reference -Object $obj -ObjectType WorkflowDefinition -Property AuthenticationWorkflowDefinition -IsArray
			$obj = Resolve-Reference -Object $obj -ObjectType WorkflowDefinition -Property AuthorizationWorkflowDefinition -IsArray
			...
			if (Test-PropertyExists -Object $obj -Property PrincipalSet) {
				$obj.PrincipalSet = $obj2.PrincipalSet
			}
			if (Test-PropertyExists -Object $obj -Property ResourceCurrentSet) {
				$obj.ResourceCurrentSet = $obj2.ResourceCurrentSet
			}
			if (Test-PropertyExists -Object $obj -Property ResourceFinalSet) {
				$obj.ResourceFinalSet = $obj2.ResourceFinalSet
			}
			if (Test-PropertyExists -Object $obj -Property ActionWorkflowDefinition) {
				$obj.ActionWorkflowDefinition = $obj2.ActionWorkflowDefinition
			}
			if (Test-PropertyExists -Object $obj -Property AuthenticationWorkflowDefinition) {
				$obj.AuthenticationWorkflowDefinition = $obj2.AuthenticationWorkflowDefinition
			}
			if (Test-PropertyExists -Object $obj -Property AuthorizationWorkflowDefinition) {
				$obj.AuthorizationWorkflowDefinition = $obj2.AuthorizationWorkflowDefinition
			}
			if ($Anchor -contains "BoundAttributeType" -and $Anchor -contains "BoundObjectType") {
...
Function Resolve-Reference {
	<#
.SYNOPSIS
Resolve reference. Store destination object ID in delta xml object.

.DESCRIPTION
Searches for the given property in the destination object and copies the object ID.
...
#>
	...
	if (Test-PropertyExists -Object $Object -Property $Property) {
		if ($IsArray) {
			$targets = @()
			foreach ($item in $Object.$Property) {
				$sourceObj = $global:SourceObjects.$ObjectType | Where-Object { $_.ObjectID.Value -eq $Object.$Property.Value }
				$targetObject = $global:TargetObjects.$ObjectType | Where-Object { $_.$Anchor -eq $sourceObj.$Anchor }
				$targets += $targetObject.ObjectID 
			}
			$Object.$Property = $targets
		}
		elseif ($null -ne $Object.$Property) {
			$sourceObj = $global:SourceObjects.$ObjectType | Where-Object { $_.ObjectID.Value -eq $Object.$Property.Value }
			$targetObject = $global:TargetObjects.$ObjectType | Where-Object { $_.$Anchor -eq $sourceObj.$Anchor }
			$Object.$Property = $targetObject.ObjectID
		}
	}
	return $Object
}
